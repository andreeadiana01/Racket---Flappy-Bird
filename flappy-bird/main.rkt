...
(define-struct bird-dup (x y v-y color))
(define-struct pipe-dup (x gap))
(define-struct state-dup (birdie pipes score))

(define (get-initial-state)
  (define bird-init (bird-dup bird-x bird-initial-y 0 "yellow"))
  (define pipe-init (pipe-dup scene-width (+ added-number (random random-threshold))))
  (define state-init (state-dup bird-init (list pipe-init) 0))
  state-init)
...
(define (next-state-bird bird gravity)
  (struct-copy bird-dup bird 
               [v-y (+ gravity (get-bird-v-y bird))] [y (+ (get-bird-y bird) (get-bird-v-y bird))]))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy bird-dup bird [v-y (- momentum)]))
...
(define (change current-state pressed-key)
  (match-let ([(state-dup birdie _ _ ) current-state])
    (if (key=? pressed-key " ")
        (struct-copy state-dup current-state [birdie (next-state-bird-onspace birdie initial-momentum)])
        current-state)))
...
(define (move-pipes pipes scroll-speed)
  (map(lambda (pipe)(match-let ([(pipe-dup x _)  pipe])(struct-copy pipe-dup pipe [x (- x scroll-speed)])))pipes))
...
(define (clean-pipes pipes)
  (filter (lambda (pipe)(match-let ([(pipe-dup x _)  pipe])(< 0 (+ pipe-width x))))pipes))
...
(define (add-more-pipes pipes)
  (if (>= (length pipes) no-pipes)
      pipes
      (cons (pipe-dup (+ pipe-width (+ (get-pipe-x (car pipes)) pipe-gap ))
...
(define (invalid-state? state)
  (if (eq? (check-ground-collision (get-bird state) ) #f)
      (if(eq? (check-pipe-collisions (get-bird state)  (get-pipes state)) #f)
         #f
         #t)
      #t))
...
(define (birdA1 bird) (make-posn (get-bird-x bird) ( get-bird-y bird)))
(define (birdA2 bird) (make-posn (+ (get-bird-x bird) bird-width) (+ (get-bird-y bird) bird-height)))
(define (pipeupB1 pipe) (make-posn (get-pipe-x pipe) 0))
(define (pipeupB2 pipe) (make-posn  (+ (get-pipe-x pipe) pipe-width) (get-pipe-gap pipe )))
(define (pipedownB1 pipe) (make-posn (get-pipe-x pipe) (+ (get-pipe-gap pipe) pipe-self-gap )))
(define (pipedownB2 pipe) (make-posn  (+ (get-pipe-x pipe) pipe-width) ground-y))

(define (check-pipe-collisions bird pipes)
  (if(null? pipes)
     #f  
     (if (eq? (check-collision-rectangles (birdA1 bird)(birdA2 bird)(pipeupB1 (car pipes))(pipeupB2 (car pipes))) #f)
         (if (eq? (check-collision-rectangles (birdA1 bird)(birdA2 bird)(pipedownB1 (car pipes))(pipedownB2 (car pipes))) #f)
...
(define (next-state state)
  (struct-copy state-dup state [birdie (next-state-bird (get-bird state) initial-gravity)]
...
(define (compute-bird-x state)( + (quotient bird-width 2) (get-bird-x (get-bird state))))
(define (compute-bird-y state)( + (quotient bird-height 2) (get-bird-y (get-bird state))))
(define (compute-ground-x state)(quotient scene-width 2))
(define (compute-ground-y state)(- scene-height (quotient ground-height 2)))


(define (draw-frame state)
  ( place-image bird-image (compute-bird-x state)(compute-bird-y state)
                (place-image ground-image (compute-ground-x state)(compute-ground-y state)
...
(define (compute-pipe-x pipe) (+ (quotient pipe-width 2) (get-pipe-x pipe)))
(define (compute-pipe-gap1 pipe) (- (get-pipe-gap pipe) (quotient pipe-height 2)))
(define (compute-pipe-self-gap1 pipe) (+ pipe-self-gap (+ (quotient pipe-height 2) (get-pipe-gap pipe))))

(define (place-pipes pipes scene)
  (if (empty? pipes)
      scene
      (place-pipes (cdr pipes) (place-image pipe-image (compute-pipe-x (car pipes))(compute-pipe-gap1 (car pipes))
                                            (place-image pipe-image (compute-pipe-x (car pipes))(compute-pipe-self-gap1 (car pipes))scene)))))
